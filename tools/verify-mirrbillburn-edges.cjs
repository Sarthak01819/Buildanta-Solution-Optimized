...
const vendorTexture = request => request.resourceType() === 'fetch'
  && new URL(request.url()).pathname === '/src/vendor/mirrbillburn/assets/leather-money-shreds.ktx2';
...
async function openPage(browser, { reducedMotion = 'no-preference', intercept } = {}) {
  ...
  if (intercept) await page.route('**/src/vendor/mirrbillburn/assets/leather-money-shreds.ktx2*', intercept);
...
(async () => {
  ...
  try {
    ...
    if (!selectedCase || selectedCase === 'missing') {
      ...
      try {
        ...
        assert.equal(aborted, 1, 'exactly the supplied KTX2 atlas fetch must fail');
        ...
        console.log('PASS missing KTX2 atlas: failure persists below burn start; next portal remains reachable.');
      } catch (error) {
        console.error('FAIL missing KTX2 atlas:', error.message, JSON.stringify(await state(page)));
        failures.push(error);
    ...
    if (!selectedCase || selectedCase === 'delayed') {
      ...
      try {
        ...
        assert.equal(delayed, 1, 'exactly one supplied KTX2 atlas fetch should be delayed');
        assert.equal(playing.failed, false);
        assert.equal(playing.scene.source, 'mirrbillburn');
        assert.ok(Math.abs(playing.scene.sourceProgress - .4 * playing.scene.progress) < .00001);
        ...
        assert.deepEqual(errors, []);
        console.log('PASS delayed KTX2 atlas: direct timeline-end jump waits, then starts automatic playback.');
      } catch (error) {
        console.error('FAIL delayed KTX2 atlas:', error.message, JSON.stringify(await state(page)));
        failures.push(error);
      } finally { await page.close(); }
    }
    ...
    if (!selectedCase || selectedCase === 'reduced') {
      ...
      try {
        ...
        assert.equal(start.scene.source, 'mirrbillburn');
        await seek(page, .98);
        ...
        assert.ok(Math.abs(scrubbed.scene.sourceProgress - (.4 * scrubbed.scene.progress)) < .00001);
  ...
  if (failures.length) throw new AggregateError(failures, `${failures.length} mirrbillburn edge case(s) failed`);
