...
export function createIntro({ onProgress } = {}) {
  ...
  const consultHand = !reduced && consultHandCanvas
    ...
    : null;
  const zeroStageAudio = !reduced ? createZeroStageAudio() : null;
  ...
  function applyProgress(p) {
    ...
    if (consultZero) {
      ...
      /* ZERO MIRROR STAGE 1 (D-056): the iris reveals the actual animated
         next scene, then p parks at .788 while a dedicated raw-scroll hold
         scrubs the full green-hand/camera/garden sequence. The first 8% is
         coupled to the opening circle so the hand comes FROM its centre;
         the hold supplies the remaining 92%, including the visible curl
         adapter. The destination meadow and matched hands resolve inside
         that hold, then the existing fist-bump continues when p advances.

         Reduced motion deliberately takes the pre-D056 path: no imported
         stage is mounted, no legacy furniture is hidden, and the existing
         static WE SCALE composition remains the fallback. */
      const zeroStageLead = reduced ? 0 : smoothstep((p - 0.738) / 0.050);
      /* The source choreography owns the first 80% of the dedicated hold;
         the last 20% is reserved for the visible curl/rotation adapter. This
         keeps external progress .95 equal to ZeroMirror's final reference
         frame while still giving the fist-bump bridge a readable scroll beat. */
      const zeroStageProgress = reduced
        ? 0
        : p < ZERO_STAGE_P
          ? zeroStageLead * 0.08
          : zeroStageLocal <= 0.8
            ? 0.08 + (zeroStageLocal / 0.8) * 0.87
            : 0.95 + ((zeroStageLocal - 0.8) / 0.2) * 0.05;
      /* Scene ownership is exclusive at the seam.  A soft dissolve here
         renders two complete hand pairs and two landscapes at once because
         THE MEET's GLB hands are opaque.  The imported rig finishes on the
         exact first fist-bump pose, so switch owners on that matched frame:
         ZeroMirror owns the entire hold; THE MEET owns the very next frame. */
      const zeroStageOpacity = reduced ? 0 : (p <= ZERO_STAGE_P ? 1 : 0);
      ...
      consultZero.classList.toggle(
        "zero-stage-live",
        !reduced && Boolean(consultHand) && zeroStageOpacity > 0.002,
      );
  ...
  /* ZeroMirror Stage 1 is authored as 175 virtual viewport units, with its
     own wheel multiplier reducing that to roughly five physical screens on
     desktop. The aperture contributes ~1.73 screens here and 80% of this
     4.15-screen hold contributes another 3.32; the remaining .83 screen is
     the visible curl/rotation into Buildanta's fist-bump. */
  const zeroStageStretch = reduced ? 0 : 4.15;
  ...
  const consultPreludeShare = (ZERO_STAGE_P - HANDOVER_P1) / (FILM_P - HANDOVER_P1);
  const SEGMENTS = [
    [0, MARKET_P0, 0, null],
    [MARKET_P0, TRANSPORT_P0, arrivalStretch, null],
    [TRANSPORT_P0, MARKET_P1, transportStretch, null],
    [MARKET_P1, HANDOVER_P1, handoverStretch, null],
    ...
    [ZERO_STAGE_P, FILM_P, consultStretch * 0.85 * (1 - consultPreludeShare), null],
    [FILM_P, FILM_P, filmStretch, "film"],
    [FILM_P, 1, consultStretch * 0.15, null],
    ...
    for (const seg of SEGMENTS) {
      ...
      if (v <= seg.vh || seg === SEGMENTS[SEGMENTS.length - 1]) {
        if (seg.p1 === seg.p0) {
          ...
          if (seg.hold === "film") nextFilm = local;
  ...
  const rawForZeroStage = (target) => {
    ...
    const local = stageLocal <= 0.95
      ? ((stageLocal - 0.08) / 0.87) * 0.8
      : 0.8 + ((stageLocal - 0.95) / 0.05) * 0.2;
    let v = 0;
    for (const seg of SEGMENTS) {
