...
export function createConsultHand(canvas) {
  ...
  function render(time = 0) {
    ...
    /* While the source stage is opaque it owns the frame. Its bridge resolves
       onto THE MEET's actual seam pose (consultLocal ~= .09), then the
       existing animation continues on its original .10→.825 clock. Never
       remap this to a later pose: doing so skips most of the fist-bump and
       desynchronises its contact/title cues.

       Scene ownership changes atomically after the matched endpoint, so the
       source and destination hand pairs are never visible together. Pointer
       parallax eases back in just after the swap; this keeps the shared seam
       pose exact even when the cursor is sitting at a viewport corner. */
    const meetPointerIn = smooth((progress - 0.10) / 0.04);
    const entryOwnsFrame = entryVisibility > 0.002;
    const bridgeProgress = zeroMirrorStage.bridgeProgress;
    /* During the late adapter THE MEET itself supplies the destination
       participant and, finally, the exact destination green mesh. The source
       counterparts have already cleared at these thresholds, so ownership
       changes without ever drawing two hand pairs. */
    const meetOwnsHuman = !entryOwnsFrame || bridgeProgress >= 0.58;
    const meetOwnsGreen = !entryOwnsFrame || bridgeProgress >= 0.92;
    meet.update(
      progress,
      time,
      visibility * (1 - exit),
      ...
      pointerY * meetPointerIn,
      {
        ...
        humanOpacity: entryOwnsFrame ? smooth((bridgeProgress - 0.58) / 0.22) : 1,
        environmentMix: smooth((bridgeProgress - 0.4) / 0.45),
      },
    );

    canvas.style.opacity = String(visibility * (1 - exit));
    renderer.render(scene, camera);
    zeroMirrorStage.render({ underlayScene: scene, underlayCamera: camera });
  }
