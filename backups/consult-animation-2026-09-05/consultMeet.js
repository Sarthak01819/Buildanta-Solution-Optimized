...
export function createConsultMeet(scene, _opts = {}) {
  ...
  group.add(rigGroup);
  let greenHandRoot = null;
  let humanHandRoot = null;
  ...
  gltfLoader.load(HANDSHAKE_URL, (gltf) => {
    ...
    greenHandRoot = root.getObjectByName("Hand_Green");
    humanHandRoot = root.getObjectByName("Hand_Human");
    root.traverse((o) => {
      if (o.isMesh) {
  ...
  function update(
    ...
    /* Establish the meadow during the incoming adapter. Replaying its old
       entrance fade after the matched handoff brightened the whole viewport
       abruptly at .788→.790, especially on shorter desktop viewports. */
    const bgOn = on * Math.max(
      smooth((p - 0.06) / 0.06),
      clamp01(ownership?.environmentMix ?? 0),
    );
    ...
    /* The ZeroMirror adapter can own the foreground hands while this exact
       destination meadow remains live underneath. Keeping those concerns
       separate prevents a second opaque GLB pair from bleeding through a
       fading source environment. */
    const handsOwnedHere = ownership?.hands !== false;
    const greenOwnedHere = ownership?.green ?? handsOwnedHere;
    const humanOwnedHere = ownership?.human ?? handsOwnedHere;
    rigGroup.visible = (greenOwnedHere || humanOwnedHere) && handsIn > 0.01;
    if (greenHandRoot) greenHandRoot.visible = greenOwnedHere;
    if (humanHandRoot) humanHandRoot.visible = humanOwnedHere;
    const humanOpacity = clamp01(ownership?.humanOpacity ?? 1);
    humanHandRoot?.traverse((object) => {
      if (!object.isMesh) return;
      object.material.opacity = humanOpacity;
      const transparent = humanOpacity < 1;
